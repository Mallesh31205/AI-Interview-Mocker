...
function Feedback({ params }) {
  ...
  return (
    <div className="p-10">
      {feedbackList.length === 0 ? (
        ...
        <>
          ...
          <h2 className="text-blue-600 font-medium text-lg my-3">
            Your Overall Interview Rating: <strong>{overallRating ?? "N/A"}</strong>
          </h2>
          ...
          {feedbackList.map((item) => (
            <Collapsible key={item.id}>
              <CollapsibleTrigger className="p-2 bg-secondary flex justify-between rounded-lg my-2 text-left">
                ...
                <ChevronsUpDown className="h-12 w-14 text-blue-500 hover:text-red-500" />
              </CollapsibleTrigger>
              <CollapsibleContent className="mt-7">
                <div className="flex flex-col gap-3">
                  <h2 className="text-black p-2 rounded-lg border bg-teal-100">
                    <strong>Rating:</strong> <br />
                    {item.rating}
                  </h2>
                  <h2 className="p-2 border rounded-lg bg-pink-100 text-sm text-black">
                    <strong>Your Answer:</strong>
                    <br />
                    {item.userAns}
                  </h2>
                  <h2 className="text-black p-2 rounded-lg border bg-green-200">
                    <strong>Correct Answer:</strong> <br />
                    {item.correctAns}
                  </h2>
                  <h2 className="text-black p-2 rounded-lg border bg-blue-200">
      ...
      <Button onClick={() => router.replace("/dashboard")} className="mt-8 bg-blue-500 w-32 h-11">
        Go Home
      </Button>
    </div>
  );
